...
class Generator(nn.Module):
  def __init__(self, G_ch=64, dim_z=128, bottom_width=4, resolution=128,
    ...
    self.dog_cnt_shared=(self.which_embedding(4, self.dim_dog_cnt_z) if G_shared
                    else layers.identity())
    # First linear layer
    ...
    if self.add_style:
      ...
      for i in range(style_mlp):
        style_layers.append(layers.StyleLayer(self.dim_z,self.which_linear,self.activation))
  ...
  def forward(self, z, y):
    ...
    if self.mix_style:
      z=z.view(z.size(0),-1,self.dim_z)
      z_p1=z[:,0]
      z_p2=z[:,1]
    else:
      z_p1=z
    if self.hier:
      zs = torch.split(z_p1, self.z_chunk_size, 1)
      z0 = zs[0]
    ...
    if self.add_style:
      styles = []
      if self.mix_style:
        inject_index=np.random.choice(list(range(n_block)),1)
        style1 = self.style(z_p1)
        if self.skip_z:
          style1 = torch.cat([style1, z_p1], -1)

        style2 = self.style(z_p2)
        if self.skip_z:
          style2 = torch.cat([style2, z_p2], -1)
        for bi in range(n_block):
          if bi<=inject_index:
            style_z=style1
          else:
            style_z=style2
          styles.append(style_z)
      else:
        style=self.style(z_p1)
        if self.skip_z:
          style=torch.cat([style,z_p1],-1)
        styles=[style]*n_block
    else:
      styles=[None] * n_block
    # Loop over blocks
    for index, blocklist in enumerate(self.blocks):
      ...
      for block in blocklist:
        h = block(h, ys[index],styles[index])
...
class Discriminator(nn.Module):
  ...
  def __init__(self, D_ch=64, D_wide=True, resolution=128,
    ...
    if self.use_dog_cnt:
      ...
      self.embed_dog_cnt = self.which_embedding(4, self.arch['out_channels'][-1])
  ...
  def forward(self, x, y=None):
    ...
    if self.use_dog_cnt:
      out = out + torch.sum(self.embed(y[:, 0]) * h, 1, keepdim=True)
      out = out + torch.sum(self.embed_dog_cnt(y[:,1]) * h, 1, keepdim=True)
    else:
...
class G_D(nn.Module):
  ...
  def forward(self, z, gy, x=None, dy=None, train_G=False, return_G_z=False,
    ...
    with torch.set_grad_enabled(train_G):
      ...
      if self.G.use_dog_cnt and self.G.G_shared:
        ye0=self.G.shared(gy[:,0])
        ye1=self.G.dog_cnt_shared(gy[:,1])
