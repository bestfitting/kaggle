...
class InceptionResnetV2(nn.Module):
    ...
    def __init__(self, num_classes=1001):
        ...
        shinke_c = 4096
        self.conv2d_7b = BasicConv2d(2080, shinke_c, kernel_size=1, stride=1)
        self.avgpool_1a = nn.AdaptiveAvgPool2d(1)
        self.fc_p1 = nn.Linear(shinke_c, shinke_c)
        self.fc_p1_bn = nn.BatchNorm1d(shinke_c)
        self.classif = nn.Linear(shinke_c, num_classes)

        self.pretrained_params = [self.conv2d_1a,self.conv2d_2a,self.conv2d_2b,self.conv2d_3b,self.conv2d_4a]
        self.pretrained_params.extend([self.mixed_5b,self.repeat,self.mixed_6a,self.repeat_1,self.mixed_7a])
        self.pretrained_params.extend([self.repeat_2,self.block8])
        self.new_params = [self.classif]
        self.new_params.append(self.conv2d_7b)
        self.new_params.append(self.fc_p1)
        self.new_params.append(self.fc_p1_bn)
    def forward(self, x):
        x = self.conv2d_1a(x)
        x = self.conv2d_2a(x)
        x = self.conv2d_2b(x)
    ...
    def load_pretrain_pytorch_file(self, pytorch_file):
        skip= ['conv2d_7b.conv.weight', 'conv2d_7b.bn.weight', 'conv2d_7b.bn.bias', 'conv2d_7b.bn.running_mean', 'conv2d_7b.bn.running_var', 'classif.weight', 'classif.bias']
        load_pretrain_pytorch_file(self,pytorch_file, skip)




def inceptionresnetv2_kxv4(num_classes=1000,**kwargs):
    model = InceptionResnetV2(num_classes,**kwargs)
    return model
...
if __name__ == "__main__":
    net = eval("inceptionresnetv2_kxv4")(num_classes=5270)
